...
1. ローカルで Ollama を起動し、モデルを用意する（デフォルト設定は `qwen3:8b`）
...
2. アプリを起動し、バルーンの入力欄から話しかけてください
3. Ollama 未起動時はバルーンにエラーが表示されます

設定は `electron-store` によりユーザーデータに保存されます（管理画面はフェーズ4）。
