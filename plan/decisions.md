...
## 2026-07-12 — キャラのレイアウト基準をウィンドウから分離（B-007/B-008）

- **問題**: キャラが実ウィンドウにフィットするレイアウトだと、B-007（ウィンドウ拡大でバルーン可動域を広げる）を入れたときにキャラまで大きくなり、B-008（スケールとウィンドウの独立制御）と両立しない。
- **採った対応**: `layoutModel` のフィット計算を固定の基準ボックス（420×880、従来の既定ウィンドウ内寸）基準に変更。キャラの画面上サイズは `character.scale` のみで決まり、ウィンドウサイズはバルーン可動域の広さとしてだけ機能する。収まらない場合はレンダラ要求（`mascot:ensureWindowSize`）でウィンドウを自動拡大（拡大のみ・下端中央固定）。
- **影響範囲**: `stage.ts` / `mascot/main.ts` / `mascotWindow.ts` / `main/index.ts` / 挙動タブ。サイズ計算は `shared/windowBounds.ts` に切り出しユニットテストを追加。既存ユーザーへの見た目の変化: ウィンドウサイズが既定420×880のままなら従来と同一。
